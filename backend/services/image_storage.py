...
class ImageStorageService:
    """Product image upload — validates the file, generates a thumbnail, and stores both to
    local disk (default, fully functional with zero credentials) or S3 (once s3_enabled is set).
    See config/settings.py for the S3_* flags."""
    ...
    @staticmethod
    async def upload(file: UploadFile, base_url: str) -> Tuple[str, str]:
        ...
        if settings.s3_enabled:
            return await ImageStorageService._upload_to_s3(key, file_bytes, thumb_bytes, file.content_type)
        return ImageStorageService._upload_to_disk(key, file_bytes, thumb_bytes, base_url)

    @staticmethod
    def _upload_to_disk(key: str, file_bytes: bytes, thumb_bytes: bytes, base_url: str) -> Tuple[str, str]:
        ...
        return f"{base}/uploads/products/{key}", f"{base}/uploads/products/thumbs/{thumb_key}"

    @staticmethod
    async def _upload_to_s3(key: str, file_bytes: bytes, thumb_bytes: bytes, content_type: str) -> Tuple[str, str]:
        ...
        full_key = f"products/{key}"
        thumb_key = f"products/thumbs/{Path(key).stem}_thumb.jpg"
